...
class ContentPane (gtk.Notebook):
    ...
    def _construct_tab_view (self, web_view, url=None):
        ...
        self.web_view.execute_script(open('jquery.js').read())

        #ถ้ายังไม่เรียกใช้ สคริปที่เสีย หรือหลัังจากสคริปที่เสีย ก็ใช้ได้,
        #Bug : แต่ถ้าเรียกไปแล้ว ทั้งก่อนและหลังจะไม่รีนทั้งหมด แม้จะเปิดแล้วปิดใหม่ก็ตาม                          
       ...
       function  pyReturn (varName) { //varName=varToReturn - STRING TYPE
        ...
        function open () {
            ...
            py("objList["+this.id+"]=open('"+fn+"', '"+mod+"')");

            this.read =function() {
                py("_out=objList["+this.id+"].read()");
                return pyReturn("_out");
            }
        }
        
        open.prototype=new _new();

var o=new open("./jquery.js");
...
if __name__ == "__main__":
    ...
    def js (javascript_code) :
       ...
       webbrowser.content_tabs.web_view.execute_script(javascript_code)

    def pyVar(varName):
        js(varName+" = "+eval(varName))
